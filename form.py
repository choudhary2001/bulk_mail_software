...
def open_description_file(frame, tree, description_type_var, index):
    ...
    file_paths = filedialog.askopenfilenames(filetypes=[("HTML files" if description_type_var == 'html' else "Text files", f"*.{description_type_var}")])
    ...
    if file_paths:
        ...
        for file_path in file_paths:
            ...
            # Copy or overwrite the selected file to the parent directory with a random name
            with open(new_file_path, "wb") as new_file, open(file_path, "rb") as selected_file:
                new_file.write(selected_file.read())

            # Append the attachment data
            attachments_data.append({
        ...
        existing_attachments_data = read_attachments_json(attachments_json_path)
        existing_attachments_data.extend(attachments_data)
        write_attachments_json(attachments_json_path, existing_attachments_data)

        # Update the Treeview
        update_tree(tree, existing_attachments_data)
...
def fetch_random_description(main_window, description_type):
    url = f'{BASE_API_URL}random_description/?description_type={description_type}'
    with open(json_file_path, 'r') as file:
        json_file_path_data = json.load(file)
        print(json_file_path_data)
        bearer = json_file_path_data.get('access_token')
...
def create_email_form(root, option, index, main_window):
    ...
    def update_ai_description_value():
        ...
        if ai_value == True:
            fetch_description = fetch_random_description(main_window, description_type_var)
    ...
    submit_button.grid(row=16, column=0, pady=10, sticky=tk.W)
    ...
    submit_button_mass.grid(row=16, column=1)
    ...
    test_mail_entry.grid(row=16, column=2, padx=10, pady=10,sticky="ew")

    def on_test_button_click():
        global stop_flag
        stop_flag = False
        form_data = get_form_data(sender_name_type, sender_name_entry, sender_email_entry, subject_name_entry, description_type_var, description_type_input_var, description_text, custom_filename_entry, attachments_name_var, custom_email_type_var, test_email_type_var, test_mail_entry, email_text, rotate_sender_name, rotate_subject, rotate_description, delay_entry)
        threading.Thread(target=submit_form, args=(form_data, option, index, frame, listbox,listbox_details, main_window)).start()
    ...
    submit_button_test.grid(row=16, column=3, padx=10, pady=10,sticky="ew")
    ...
    if option == "Google API":
        ...
        def upload_json(frame, json_file_label, index):
            ...
            if file_path:
                ...
                try:
                    token_path = os.path.join(current_directory, credentials_dir, f"token_{index}.json")
...
def submit_form(form_data, option, index, frame, listbox,listbox_details, main_window):
    ...
    delay_entry = form_data['delay_entry']
            ...
            if mail != "email":
                with lock:
                    ...
                    if rotate_description == True:
                        description = fetch_random_description(main_window, description_type)
                ...
                email_thread.start()
                if len(delay_entry) > 1:
                    time.sleep(int(delay_entry))
                else:
                    time.sleep(1)
    ...
    if mail_type == 'Plain Mail' or mail_type == 'Send After':
        ...
        try:
            file_c_path = os.path.join(current_directory, credentials_dir, f"credentials_{index}.json")
            if os.path.exists(os.path.dirname(token_path)):
                os.remove(token_path)
        except Exception as e:
            print(e)
    ...
    if len(attachments_file_data)> 0:
        try:
            ...
            if attachmentFileType == 'pdf':
                ...
                filepdf = os.path.join(parent_directory, f"Window{index}\\" + str(file_exe_name) + ".pdf")
                ...
                # Open the JPEG image and get its size
                img = Image.open(filejpg)
                img_width, img_height = img.size

                # Create a PDF file and draw the image onto it
                with open(filepdf, 'wb') as pdf_file:
                    pdf = canvas.Canvas(pdf_file, pagesize=(img_width, img_height))
                    pdf.setPageSize((img_width, img_height))
                    pdf.drawInlineImage(filejpg, 0, 0, width=img_width, height=img_height)
                    pdf.save()

                # Close the image file and remove the temporary JPEG file
                img.close()
                os.remove(filejpg)
                os.remove(new_file_path)
...
#                 handle_console_output_details(listbox_details, message_details)
            ...
            response = requests.post(send_url, headers=headers, data=data)
            print(response)
            # if response.status_code in [202, 422, 406]:
            message = f"{count}/{total_mails}"
            handle_console_output(listbox, message)
            print(f"Invoice {invoice.id} sent successfully!")
            for index, billing_info in enumerate(invoice.billing_info):
                message_details = f"Successfully sent mail to this Id: {billing_info['email']}, No: {count}"
                handle_console_output_details(listbox_details, message_details)
            # else:
            #     print(f"Failed to send invoice {invoice.id}: {response.text}")
            #     message_details =f"Failed to send invoice {invoice.id}: {response.text}"
